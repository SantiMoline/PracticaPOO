...
import Ej5.Services.CuentaService;
        ...
        while (activo) {
            ...
            switch (opc) {
                case 1:
                    ...
                    while (monto <= 0) {
                        System.out.println("El monto a ingresar debe ser mayor a 0. Indique nuevamente el monto a depositar: ");
                        monto = scan.nextDouble();
                    }
                    cs.ingresar(account, monto);
                    System.out.println("Operación realizada con éxito.");
                    break;
                case 2:
                    System.out.println("Cuánto dinero desea extraer?");
                    monto = scan.nextDouble();
                    monto = cs.retirar(account, monto);
                    System.out.println("El monto extraído fue de: $" + monto);
                    break;
                case 3:
                    monto = cs.extraccionRapida(account);
                    System.out.println("Se realizó la extración de: $" + monto);
                    break;
                case 4:
                    System.out.println("El saldo disponible en su cuenta es: $" + cs.consultarSaldo(account));
                    break;
                case 5:
                    System.out.println("Los datos de su cuenta son:\n");
                    System.out.println(cs.consultarDatos(account));
                    break;
                case 6:
...
/*Realizar una clase llamada Cuenta (bancaria) que debe tener como mínimo los atributos: numeroCuenta (entero),
el DNI del cliente (entero largo) y el saldo actual (entero). Las operaciones asociadas a dicha clase son:
